...
fs.readFile(file_path, 'utf8', (err, data) => {
	...
	if(!isEmptyObject(data)){
		...
		if(datajson.notify < microtime){
			notify = microtime + 86400000; //86400000 - one day
			lock = true;
			thanks();
		}else{
		...
		if(datajson.alert === false){
			alert = microtime + 259200000; //259200000 - one time in three days
...
function isEmptyObject(obj) {
  ...
  function AlertUpdate(){
		...
		console.log(' Use: [npm install compress-images --save-]');
		console.log(colors.yellow(' ============================================'));
		console.log(colors.yellow(' ============================================'));
		console.log(colors.yellow(' ============================================'));  	
  }
...
function exec_command0() {
	...
	function alert(){
		...
		if(platform == 'win32'){
	    	...
	    	exec('start cmd.exe /K node alert.js', (error, stdout, stderr) => {
